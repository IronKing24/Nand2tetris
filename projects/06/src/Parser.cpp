...
void Parser::advance()
{
    ...
    std::string::iterator end_ite = (end_offset == std::string::npos)
					? token.end()
					: token.begin() + end_offset;
...
const std::string Parser::symbol()
{
    ...
    case instructionTypes::C_INSTRUCTION:
	throw std::runtime_error(
	    "The current instruction is not a lable or an address:\n" +
	    current_instruction);
    }
}
...
{
    if (instructionType() !=
	HackAssembler::Parser::instructionTypes::C_INSTRUCTION)
    {
