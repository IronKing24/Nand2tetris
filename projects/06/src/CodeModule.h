...
namespace HackAssembler 
{
	...
	{
	public:
		///<summary>Returns the binary code for <paramref name="dest"/> mnemonic.</summary>
		///<returns>3 bits, as a string.</returns>
		const std::string dest(const std::string& dest);

		///<summary>Returns the binary code for <paramref name="comp"/> mnemonic.</summary>
		///<returns>7 bits, as a string.</returns>
		const std::string comp(const std::string& comp);

		///<summary>Returns the binary code for <paramref name="jump"/> mnemonic.</summary>
		///<returns>3 bits, as a string.</returns>
	...
	private:
		const std::unordered_map<std::string, const std::string> dests = {
			...
			{"AM"  , "101"}, {"MA"  , "101"},
			{"AD"  , "110"}, {"DA"  , "110"},
			{"ADM" , "111"}, {"AMD" , "111"}, {"MAD" , "111"},
			{"MDA" , "111"}, {"DAM" , "111"}, {"DMA" , "111"}
		};
