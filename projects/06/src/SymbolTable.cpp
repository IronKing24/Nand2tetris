...
SymbolTable::~SymbolTable() { SymbolTable::symbol_table.clear(); }
...
void SymbolTable::addEntry(const std::string &symbol, const uint16_t address)
{
	...
	throw std::runtime_error(
	    "Buffer overflow caused by symbol: " + symbol +
	    "and an address value of: " + std::to_string(address));
    }
...
{
    return SymbolTable::symbol_table.find(symbol) !=
	   SymbolTable::symbol_table.cend();
}
